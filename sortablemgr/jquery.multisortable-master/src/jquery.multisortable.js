...
!function($) {
		...
		return this.each(function() {
			...
			if (!list.data('multiselectable')) {
				list.data('multiselectable', true)
					.on('mousedown', options.items, mouseDown)
					.on('click', options.items,click);
	...
	$.fn.multisortable = function(options) {
		...
		function regroup(item, list) {
			var selclass = list.data('selectedClass') || settings.selectedClass;
			if (list.find('.' + selclass).length > 0) {
				var myIndex = item.data('i');

				var itemsBefore = list.find('.' + selclass).filter(function() {
				...
				var itemsAfter = list.find('.' + selclass).filter(function() {
					return $(this).data('i') > myIndex
				}).css({
						position: '',
						width: '',
				...
				setTimeout(function() {
					itemsAfter.add(itemsBefore).addClass(selclass);
				}, 0);
			}
		}
		...
		return this.each(function() {
			...
			//enable sorting
			options.cancel = settings.items + ':not(.' + selClass + ')';
			options.placeholder = settings.placeholder;
			options.start = function(event, ui) {
				if (ui.item.hasClass(selClass)) {
					var parent = ui.item.parent();

					//assign indexes to all selected items
					parent.find('.' + selClass).each(function(i) {
						$(this).data('i', i);
					});

					// adjust placeholder size to be size of items
					var height = parent.find('.' + selClass).length * ui.item.outerHeight();
			...
			options.sort = function(event, ui) {
				...
				var height = 0;
				$('.' + selClass, parent).filter(function() {
					return $(this).data('i') < myIndex;
